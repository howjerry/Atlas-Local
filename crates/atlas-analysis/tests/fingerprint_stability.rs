...
fn make_finding(rule_id: &str, file_path: &str, snippet: &str) -> atlas_analysis::Finding {
    FindingBuilder::new()
        ...
        .file_path(file_path)
        .line_range(atlas_analysis::LineRange {
            start_line: 10,
            start_col: 1,
            end_line: 10,
            end_col: 50,
...
fn fingerprint_stable_on_line_drift() {
    ...
    assert_eq!(
        ...
        "line drift must not change fingerprint"
    );
}
...
fn fingerprint_stable_on_unrelated_edit() {
    ...
    assert_eq!(
        f1.fingerprint, f2.fingerprint,
        "unrelated edit must not change fingerprint"
    );
}
...
fn fingerprint_changes_on_file_rename() {
    ...
    assert_ne!(
        f1.fingerprint, f2.fingerprint,
        "file rename must change fingerprint"
    );
}
...
fn fingerprint_deterministic_across_calls() {
    ...
    for _ in 0..10 {
        let f = make_finding(
            "atlas/security/typescript/code-injection",
            "src/code.ts",
            "Function(userInput)();",
        );
